...
function addUsage(current: typeof emptyUsage, next: typeof emptyUsage) {
  return {
...
const understandRequestStep = createStep({
  ...
  execute: async ({ inputData, suspend, resumeData }) => {
    ...
    if (prompt && (!url || !format)) {
      ...
      url ??= extractedRequest.output.url ?? context?.url ?? null;
      format ??= extractedRequest.output.format ?? context?.format ?? null;
    } else {
...
export const summarizePageStep = createStep({
  ...
  execute: async ({ inputData }) => {
    ...
    if (inputData.format === 'text') {
      ...
      const usage = addUsage(inputData.usage, normalizeUsage(result.usage));
