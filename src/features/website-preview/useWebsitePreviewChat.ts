...
import { consumeAgentStream, consumeWorkflowStream } from './stream';
import type {
  AIUsage,
  Approach,
...
import { addAIUsage, EMPTY_AI_USAGE, subtractAIUsage } from './types';
...
export function useWebsitePreviewChat() {
  ...
  const [totalUsage, setTotalUsage] = useState<AIUsage>(EMPTY_AI_USAGE);
  ...
  function updateLastAssistantMessage({
    ...
    setMessages((prev) =>
      prev.map((chatMessage, index) => {
        if (chatMessage.role === 'assistant' && index === prev.length - 1) {
          return {
            ...chatMessage,
            content,
            format,
  ...
  function addToTotalUsage(usage: AIUsage) {
    setTotalUsage((currentUsage) => addAIUsage(currentUsage, usage));
  }

  function updateWorkflowRequestContext(context: WorkflowRequestContext) {
    setWorkflowRequestContext((currentContext) => ({
      url: context.url ?? currentContext.url,
      format: context.format ?? currentContext.format,
    }));
  }
